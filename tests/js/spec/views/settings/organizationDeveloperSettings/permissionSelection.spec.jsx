...
describe('PermissionSelection', () => {
  ...
  it('lists human readable permissions', () => {
    const findOptions = name => {
      openMenu(wrapper, {name});

      return wrapper.find(`SelectField[name="${name}"] Option`).map(o => o.text());
    };
    ...
    expect(findOptions('Event')).toEqual(['No Access', 'Read', 'Admin']);
    ...
    expect(findOptions('Member')).toEqual(['No Access', 'Read', 'Admin']);
  });
  ...
  it('stores the permissions the User has selected', () => {
    ...
    selectByValue(wrapper, 'write', {name: 'Project'});
    selectByValue(wrapper, 'read', {name: 'Team'});
    selectByValue(wrapper, 'admin', {name: 'Release'});
    selectByValue(wrapper, 'admin', {name: 'Event'});
    selectByValue(wrapper, 'read', {name: 'Organization'});
